...
`projects.test.mjs`는 공통·폴더 설정 파일, 재정의 제거, 폴더 별칭, 탭·창 정책, 독립 모달, 네이티브 창 닫기·다시 열기 후 배치와 창 좌표 복원을 검사한다. `settings.test.mjs`는 설정 우선순위와 브라우저 저장 트랜잭션을 검사한다. 네이티브 파일 저장 단위 검사는 `go test -C examples/wailsv3 ./...`와 `cargo test --locked --manifest-path examples/tauriv2/src-tauri/Cargo.toml`이다.
...
- 존재하는 프로젝트 폴더 두 개를 추가하고 순서를 변경한 뒤 앱을 종료하고 다시 실행한다. 저장된 첫 프로젝트만 열린다. 다른 프로젝트를 다시 열고 저장된 배치와 설정을 확인한다.
