...
네이티브 프로브의 `state`는 화면 배율과 창 위치를 반환한다. `position`은 AppKit 전역 좌표를 사용한다. `mouse`는 콘텐츠 영역의 좌측 상단 좌표와 `down` 또는 `up` 단계를 사용한다. 두 명령 모두 앱을 활성화하지 않는다.
