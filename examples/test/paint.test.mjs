 ...
 * 흔들 경계와 폭.
 *
 * 예제가 처음 그리는 배치는 정해져 있다. 터미널 카드의 왼쪽 세로 경계를 잡고
 * 왼쪽으로 250px, 누른 채로 15번 왕복한다. 터미널 표면이 그만큼 커졌다 작아진다.
 */
const DRIVE = "3000,404,294,-250,0,48,15";
...
function bare(frame) {
  let n = 0;
  for (let y = Math.round(frame.height * ROW.from); y < frame.height * ROW.to; y += 2) {
...
for (const [name, binary] of Object.entries(APPS)) {
  test(`${name}: 경계를 흔들어도 칠해지지 않은 자리가 드러나지 않는다`, async (t) => {
    const run = await shake(binary, DRIVE);
    if (!run) return t.skip(`${binary} 가 없다`);
    try {
      const files = frames(run.into);
      assert.ok(files.length > 60, `녹화된 프레임이 ${files.length} 장뿐이다:\n${run.log}`);
