...
에서, 크기 변경에서, 그리고 누름과 키와 이동 자체에서 실행된다. 호스트가 판을 바꾸고
나중에 그리면 그 사이에 그것들이 도착하기 때문이다. 그래서 호스트가 경계를 디바이더를
잡는 너비보다 멀리 옮긴 뒤에는 그 디바이더를 눌러도 아무 경계도 잡지 못한다. `commit`
훅이 그리기를 붙들고 있는 동안에는 디바이더가 뷰 자신의 변경만큼 뒤처져 있고 제스처는
그 변경을 따라 옮겨졌으므로, 그때의 누름은 평소대로 경계를 잡는다.
