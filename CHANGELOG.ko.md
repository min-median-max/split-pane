...
- 시작과 새 창에 프로젝트 라이브러리를 표시한다. 프로젝트 생성, 기존 폴더 선택, Git 저장소 복제 시 같은 미선택 OS 창을 사용한다. 셸을 닫지 않고 작업 화면에서 프로젝트 목록으로 돌아간다.
- 작업 화면 렌더러가 저장한 카드 사각형, 간격, 모서리, 레일 경로로 라이브러리 미리보기를 그린다. 프로젝트 검색, 정렬, 열림·고정·Git 필터, 고정 저장을 추가한다.
